...
begin
  ...
  SetLength(JpegBytes, FimJPEG - InicioJPEG + 1);
  Move((BufferBytes + InicioJPEG)^, JpegBytes[0], Length(JpegBytes));

  try
    CarregaJpegNoBitmap(JpegBytes, FBitmap);
    FUltimaImagemDataHora := Now;
    Result := True;
  except
    on E: Exception do
    begin
      Log('Erro ao carregar JPEG: ' + E.Message);
      Result := False;
    end;
  end;
