...
begin
  ...
  //Envia liberação de video*/
  //sendMagicPacketsVideo1();
  sleep(1000);
  Application.ProcessMessages;

  //*Envia liberacao de video segundo bloco*/
...
begin
  ...
  begin
    ...
    DataArray[2] := ord(trunc((frmMain.X +1)*127.5));
    DataArray[3] := ord(trunc(2-(frmMain.Y)+1)*127);
    DataArray[4] := ord(trunc(frmMain.B1 * 255));
    DataArray[5] := ord(trunc(frmMain.B2 +1)* 127);
    DataArray[6] := ord(trunc(checksum()));
    info := '';
    for a:= 1 to 6 do
...
begin
     ...
     Except
       meLog.Append(datetimetostr(now)+': Hand Shake fail');
       lbStateConection.Caption:= 'Fail...';
       Error := true;
     end;
end;
